...
class RKRsrnmV2(AlgorithmBase):
    ...
    def run(self, problem, logger: Logger) -> AlgorithmResult:
        ...
        for k in range(max_iters):
            ...
            x_diff_norm = (
                norm(x - prev_state["x"]) if prev_state is not None else np.inf
            )
    ...
    def _rk_inner_loop(
        *,
        ...
        for l in range(L):
            ...
            new_y = y - Q.T @ delta
            diff_norm = norm(new_y - y)
            y = new_y

            if diff_norm < x_threshold:
                break
